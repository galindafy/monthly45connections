document.addEventListener("DOMContentLoaded", () => {
  ...
  function displayWord(word) {
    return word.replace(/\s+(film|song|novel|app|platform|service|company|book|game|drink|beverage|instrument|flower|tree|fish|seafood|vehicle|state)$/i, "").replace(/\s+\[.*?\]$/, "").trim();
  }
  ...
  function createWeeklyCategories(dateKey) {
    ...
    const categories = chosen.map(([name, words], idx) => ({name, words: pickCount(words, SIZE, seed + idx * 101)}));
    const seen = new Set();
  ...
  function buildFreshState() {
    ...
    return {
      dateKey,
      lookup,
      groups: shuffle(tiles, seedNumber(dateKey) + 999).map(word => ({words:[word], solved:false, category:null, color:null})),
      selectedIndex:null,
      mistakes:0
    };
  }
