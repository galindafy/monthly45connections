document.addEventListener("DOMContentLoaded", () => {
  ...
  function buildPoolCategory(name, poolName, dateKey, offset) {
    const source = uniqueList(POOLS[poolName]);
    if (source.length < SIZE) throw new Error(`${name} does not have enough unique items.`);
    return { name, words: shuffle(source, Number(dateKey) + offset).slice(0, SIZE) };
  }

  function buildPrefixCategory(name, prefix, dateKey, offset) {
    const source = uniqueList(PREFIX_SUFFIXES[prefix]);
    const words = shuffle(source, Number(dateKey) + offset)
      .slice(0, SIZE)
      .map(s => `${prefix} ${s}`);
    if (words.length < SIZE) throw new Error(`${name} does not have enough unique items.`);
    return { name, words };
  }
  ...
  function createDailyCategories(dateKey) {
    const defs = [
      ...TEMPLATE_DEFS.map(([name, pool]) => ({ kind: "pool", name, pool })),
      ...PREFIX_TEMPLATES.map(([name, prefix]) => ({ kind: "prefix", name, prefix }))
    ];

    const picked = shuffle(defs, Number(dateKey) + 77).slice(0, SIZE);

    const categories = picked.map((def, i) => {
      return def.kind === "pool"
        ? buildPoolCategory(def.name, def.pool, dateKey, 300 + i)
        : buildPrefixCategory(def.name, def.prefix, dateKey, 700 + i);
    });
    ...
    categories.forEach(cat => {
      ...
      cat.words = cat.words.map(word => {
        if (!word) throw new Error(`Blank tile in ${cat.name}`);
        if (!seen.has(word)) {
          seen.set(word, 1);
          return word;
        }
        const count = seen.get(word) + 1;
        seen.set(word, count);
        return `${word} (${shortTag(cat.name)})`;
      });
    });
    ...
    categories.forEach(cat => {
      cat.words.forEach(word => {
        if (finalSet.has(word)) throw new Error(`Duplicate tile after uniquify: ${word}`);
        finalSet.add(word);
      });
    });
  ...
  function buildFreshState() {
    ...
    const categories = createDailyCategories(dateKey);
    ...
    return {
      dateKey,
      lookup,
      groups: shuffle(tiles, Number(dateKey) + 999).map(word => ({
        words: [word],
        solved: false,
        category: null,
        color: null
  ...
  function isValidSavedState(saved, fresh) {
    if (!saved || saved.dateKey !== fresh.dateKey || !Array.isArray(saved.groups) || !saved.groups.every(isValidGroup)) return false;
  ...
  function render() {
    dateEl.textContent = formatDateKey(state.dateKey);
  ...
  shuffleBtn.addEventListener("click", () => {
    ...
    state.groups = [...solved, ...shuffle(unsolved, Number(state.dateKey) + (Date.now() % 1000))];
    state.selectedIndex = null;
    saveState();
    render();
  });
